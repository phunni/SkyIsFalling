...
public class Box2dLoader {
    ...
    public Box2dLoader(FileHandle file) {
...
//		OrderedMap<String,?> originElem = (OrderedMap<String,?>) bodyElem.get("origin");
        ...
        for (JsonValue poly = polygons.child; poly != null; poly = poly.next) {
            PolygonModel polygon = new PolygonModel();
            rbModel.polygons.add(polygon);

            for (JsonValue vert = poly.child; vert != null; vert = vert.next) {
                polygon.vertices.add(new Vector2(vert.getFloat("x"), vert.getFloat("y")));
            }
